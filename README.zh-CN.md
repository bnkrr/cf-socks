...
如果一次性协议需要在发送 payload 后关闭目标写侧，可以显式启用：
...
这会发送空 payload，等待 200ms，然后关闭目标 writable side，同时继续读取响应。它适合 SSH banner 这类 bounded probe。默认行为是不关闭目标写侧。最大延迟是 10 分钟；更长生命周期的响应应保持该选项未设置。
...
`Do(ctx, "tcp", "github.com:22", nil)` 会发送空 payload，H2 或 H3 都可读取 SSH 这类 server-first banner；但它仍然不是交互式连接。只有当目标写侧 EOF 是一次性交换语义的一部分时，才使用 `WithWriteCloseAfter`。
...
- H2/H3 模式只支持 bounded payload；它不是 SOCKS 或 `net.Conn` transport。`WithWriteCloseAfter` 可以为一次性交换关闭目标写侧，但不会让 H2/H3 变成交互式连接。
